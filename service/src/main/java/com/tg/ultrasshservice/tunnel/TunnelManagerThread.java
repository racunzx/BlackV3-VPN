...
import com.tg.ultrasshservice.tunnel.vpn.TunnelVpnService;
import androidx.localbroadcastmanager.content.LocalBroadcastManager;
import java.util.List;
import com.tg.ultrasshservice.tunnel.vpn.VpnUtils;
import android.util.Log;
import com.tg.ultrasshservice.tunnel.vpn.TunnelState;
import android.content.Intent;
import com.tg.ultrasshservice.tunnel.vpn.TunnelVpnSettings;
import android.content.BroadcastReceiver;

import com.tg.ultrasshservice.tunnel.vpn.TunnelVpnManager;
...
import com.tg.ultrasshservice.config.PasswordCache;
	...
	protected void conectar(String servidor, int porta) throws Exception {
		...
		try {
			...
			if (useProxy && cause.contains("Key exchange was not finished")) {
				SkStatus.logError("<strong></font><font color=\"red\">" + "ERRO:CONEXÃO PERDIDA" + "</strong>");
			}
			else {
				SkStatus.logError("<strong></font><font color=\"red\">" + "ERRO:NÃO FOI POSSÍVEL COMPLETAR A CONEXÃO" + "</strong>");
	...
	protected void autenticar(String usuario, String senha, String keyPath) throws IOException {
		...
		try {
			if (mConnection.isAuthMethodAvailable(usuario,
				...
				SkStatus.logInfo("Verificando seu usuário/senha...");
		...
		if (!mConnection.isAuthenticationComplete()) {
			...
			SkStatus.logInfo("<strong></font><font color=\"red\">Entre em contato com o dono do servidor p/ mais informações </strong>");
			stopAll();
